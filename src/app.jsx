...
export default function App() {
  ...
  const [chatMessages, setChatMessages] = useState(() => JSON.parse(localStorage.getItem('chatMessages')) || [
    { id: 1, sender: 'מערכת', text: 'ברוכים הבאים לליגת יוספטל! המערכת פתוחה לניחושים.', time: new Date().toLocaleTimeString('he-IL', {hour: '2-digit', minute:'2-digit'}) }
  ]);
  const [newChatMessage, setNewChatMessage] = useState('');
  const [adminAlert, setAdminAlert] = useState(() => localStorage.getItem('adminAlert') || 'ברוכים הבאים לגרסת הבטא! נא למלא ניחושים.');
  ...
  useEffect(() => {
    const updateTimer = () => {
      ...
      if (diff <= 0) {
        ...
        setCountdownText(`⏱️ נעילת מחזור בעוד: ${days} ימים ו-${hours} שעות`);
      }
    };
    updateTimer();
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white pb-24" style={{ direction: 'rtl' }}>
      ...
      <div className="sticky top-0 bg-gray-950/95 backdrop-blur-md pt-2 pb-3 z-40 max-w-md mx-auto px-2 border-b border-gray-900/50">
        <header className="flex justify-between items-center p-3 bg-gray-900 rounded-xl border-b border-yellow-500/30 shadow-lg">
          <div>
          ...
          <div className="bg-gray-800 text-xs font-bold px-3 py-1.5 rounded-lg border border-gray-700">
            👤 {username}
          </div>
        </header>
      ...
      <div className="max-w-md mx-auto mt-4 px-2">
        ...
        {currentTab === 'tournament' && (
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4 shadow-2xl">
            ...
            <div className="pt-2">
              <label className="block text-xs font-bold text-gray-400 mb-1">💙 הקבוצה האהודה עלי:</label>
            ...
            <button type="button" onClick={() => alert('הבחירות ארוכות הטווח עודכנו בהצלחה!')} className="w-full bg-yellow-500 text-gray-950 font-black py-3.5 rounded-xl border border-yellow-600 text-sm mt-2">💾 שמור בחירות</button>
          </div>
        )}

        {currentTab === 'chat' && (
          <div className="bg-gray-900 border border-gray-800 rounded-2xl flex flex-col h-[65vh] overflow-hidden shadow-xl">
        ...
        {currentTab === 'leaderboard' && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 shadow-xl">
            <h2 className="text-lg font-bold text-gray-200 mb-3">📊 טבלת הליגה</h2>
              ...
              <table className="w-full text-right border-collapse">
                ...
                <tbody className="divide-y divide-gray-800">
                  <tr className="bg-gray-900">
                    ...
                    <td className="p-3 font-medium text-gray-100">{username}{userTeamSuffix}</td>
                    <td className="p-3 text-center font-bold text-yellow-500 bg-gray-950/40">{stats.totalPoints}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <p className="text-center text-[10px] text-gray-500 mt-4">הטבלה מסונכרנת בזמן אמת לאחר עדכון תוצאות.</p>
          </div>
        )}
      ...
      <footer className="max-w-md mx-auto mt-8 mb-4 text-center">
        {isAdminMode ? (
          <div className="bg-red-950/30 border border-red-900 p-4 rounded-xl space-y-3 mb-4 text-right">
            ...
            <p className="text-[10px] text-gray-500 text-center pt-2">עריכת תוצאות תתווסף לאחר החיבור לשרת הנתונים המרכזי.</p>
            <button onClick={loginAsAdmin} className="w-full mt-2 bg-red-900 text-white py-2 rounded font-bold text-xs">סגור פאנל ניהול</button>
          </div>
        ) : (
          <button type="button" onClick={loginAsAdmin} className="px-4 py-2 text-[10px] font-bold rounded-lg border bg-gray-900 border-gray-800 text-gray-600 hover:text-white transition-all">
            🔧 כניסת מנהל (הזנת תוצאות)
          </button>
        )}
      </footer>
    </div>
